...
- **BBR congestion control** (`TCP_CONGESTION = "bbr"`) — set on the outer TCP socket on Linux. BBR uses bandwidth and RTT estimates rather than packet loss as its primary signal, which makes it significantly more robust for tunneled traffic.
