...
<Tabs>
  <Tab title="Linux">
    DSVPN sets up a **policy routing** scheme on Linux that avoids routing loops. The VPN's own TCP socket is excluded from the policy table using a socket mark, so tunnelled traffic never recursively re-enters the VPN.
    ...
    **BBR congestion control** — enabled on the outer TCP socket for better performance:
    ```bash
    ...
    **Policy routing table 42069** — all unmarked traffic uses a separate routing table that sends everything through the TUN:
    ```bash
    ip route add default via 192.168.192.254 table 42069
    ip rule add not fwmark 42069 lookup 42069
    ip rule add fwmark 42069 lookup main suppress_prefixlength 0
    ```
    ...
    **Anti-spoof protection**
    ```bash
    iptables -t nat -I PREROUTING -d <local-tun-ip> ! -i <tun> -j DROP
    ```
  </Tab>
  <Tab title="macOS / BSD">
    ...
    **Split default routes** — covering the entire address space in two halves routes all traffic through the TUN without replacing the `0.0.0.0/0` default route:
    ```bash
    route add 0.0.0.0/1   192.168.192.254
    route add 128.0.0.0/1 192.168.192.254
    ```

    **IPv6 blackhole routes** — prevent IPv6 traffic from leaking outside the VPN:
    ```bash
    route add -inet6 0000::/1   -blackhole
    route add -inet6 8000::/1   -blackhole
    ```
  </Tab>
</Tabs>
